...
from main.database import create_notes_table, save_notes, fetch_notes, update_note, delete_notes
...
class NotesApp:
    ...
    def add_note(self):
        ...
        save_notes(task_id, title, content)
        self.load_notes()
    ...
    def show_note_details(self, event):
        ...
        note = fetch_notes(note_id)  # Modify this if fetch_notes does not return the note directly
        if note:
            note_details = f"Note Title: {note[1]}\nContent: {note[2]}"
